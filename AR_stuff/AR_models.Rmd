...
In addition to the linear models, we will also try using models with an AR component on the CO2 emission coefficient to capture possible "inertia" of CO2 emissions caused by changes in the energy infrastructure. 
...
MCMC showed no signs of divergence: we had no divergent transitions, the $\hat R$ values for all of our models are 1. Even though the effective samples sizes differ from the number of samples significantly, they still show a decent amount indicating that the sampling process had no serious divergence issues and can be used for analysis.
